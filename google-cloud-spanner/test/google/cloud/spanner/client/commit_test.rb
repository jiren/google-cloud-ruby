...
describe Google::Cloud::Spanner::Client, :read, :mock_spanner do
  ...
  it "commits using a block" do
    ...
    mock.expect :commit, commit_resp, [{
      ...
      single_use_transaction: tx_opts, request_options: nil
    }, default_options]
  ...
  it "deletes all rows directly with custom timeout and retry policy" do
    ...
    mock.expect :create_session, session_grpc, [{database: database_path(instance_id, database_id), session: nil}, default_options]
    mock.expect :commit, commit_resp, [{
      session: session_grpc.name, mutations: mutations,
      transaction_id: nil, single_use_transaction: tx_opts, request_options: nil
    }, expect_options]
    spanner.service.mocked_service = mock
  ...
  describe "commit options" do
    ...
    it "commit using block and return commits stats" do
      ...
      mock = Minitest::Mock.new
      mock.expect :create_session, session_grpc, [{database: database_path(instance_id, database_id), session: nil}, default_options]
      mock.expect :commit, commit_stats_resp_grpc, [{
        session: session_grpc.name, mutations: mutations, transaction_id: nil,
        single_use_transaction: tx_opts, return_commit_stats: true,
        request_options: nil
      }, default_options]
      spanner.service.mocked_service = mock
