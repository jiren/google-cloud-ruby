...
describe Google::Cloud::Spanner::Transaction, :batch_update, :mock_spanner do
  ...
  it "can execute a DML query with multiple statements and param types" do
    ...
    mock.expect :execute_batch_dml, batch_response_grpc(9), [{
      session: session_grpc.name, transaction: tx_selector, statements: statements, seqno: 1,
      request_options: nil
    }, default_options]
    session.service.mocked_service = mock
  ...
  describe "when used with execute_update" do
    ...
    it "increases seqno for each request" do
      ...
      mock.expect :execute_batch_dml, batch_response_grpc, [{
        session: session_grpc.name, transaction: tx_selector, statements: [statement],
        seqno: 2, request_options: nil
      }, default_options]
